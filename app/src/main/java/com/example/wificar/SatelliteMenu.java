...
public class SatelliteMenu extends ViewGroup {
    ...
    private WifiManager mWifi = MainActivity.getWifi();
    boolean wifi_switch = MainActivity.isWifi_switch();
    ...
    public void setMyOnClick(OnSatelliteMenuItemClickListener onSatelliteMenuItemClickListener) {
        mMenuItemClickListener = onSatelliteMenuItemClickListener;
    }
    ...
    public void shrinkMenu(int time) {
        ...
        for (int i = 0; i < count - 1; i++)             //便利所有按钮
        {
            ...
            //如果当前按钮菜单为关闭状态
            if (mCurrentStatus == Status.CLOSE) {   //设置打开按钮菜单动画
...
//        View childView4 = getChildAt(pos);
//        if(mMenuItemClickListener!=null){
//            mMenuItemClickListener.onClick(childView4,pos);
//        }
...
//        }
     ...
     */
    private Animation scaleSmallAnim(int time) {   //设置动画集合
     ...
     */
    private Animation scaleBigAnim(int time) {   //设置动画集合
        AnimationSet animationSet = new AnimationSet(true);
        //缩放动画
